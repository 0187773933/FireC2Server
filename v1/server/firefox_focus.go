...
const FIREFOX_FOCUS_ACTIVITY = "com.amazon.cloud9/com.amazon.slate.fire_tv.FireTvSlateActivity"
const FIREFOX_FOCUS_APP_NAME = "com.amazon.cloud9"
...
func ( s *Server ) FirefoxFocusReopenApp() {
	...
	// s.ADB.Shell( "settings" , "put" , "system" , "accelerometer_rotation" , "1" )
	// s.ADB.Portrait()
	log.Debug( "Done" )
}
...
func ( s *Server ) FireFoxFocusOpenAudioPlayer( c *fiber.Ctx ) ( error ) {
	x_url := c.Params( "*" )
	x_url_b64 := base64.StdEncoding.EncodeToString( []byte( x_url ) )
	fmt.Println( x_url_b64 )
	log.Debug( fmt.Sprintf( "FireFoxFocusOpenAudioPlayer( %s )" , x_url ) )
	...
	// target_url := fmt.Sprintf(
	// 	"%s/firefox-focus/audio" ,
	// 	s.Config.ServerPublicUrl ,
	// )
	fmt.Println( target_url )
	s.ADB.Type( target_url )
	// audio_player_path := s.Config.ServerPublicUrl + "/firefox-focus/audio"
	// s.ADB.Type( audio_player_path )
	// time.Sleep( 100 * time.Millisecond )
	// s.ADB.Type( fmt.Sprintf( "?k=%s" , s.Config.ServerAPIKey ) )
	// time.Sleep( 100 * time.Millisecond )
	// s.ADB.Type( fmt.Sprintf( "\\&url=%s" , x_url_b64 ) )
	...
	s.Set( "active_player_now_playing_id" , x_url )
	s.Set( "active_player_now_playing_uri" , "" )
	return c.SendFile( "./v1/server/html/firefox_focus_audio_player.html" )
}
...
func ( s *Server ) FireFoxFocusOpenVideoPlayer( c *fiber.Ctx ) ( error ) {
	x_url := c.Params( "*" )
	log.Debug( fmt.Sprintf( "FireFoxFocusOpenVideoPlayer( %s )" , x_url ) )
	s.FirefoxFocusReopenApp()
	time.Sleep( 1000 * time.Millisecond )
	s.ADB.Type( x_url )
	time.Sleep( 500 * time.Millisecond )
